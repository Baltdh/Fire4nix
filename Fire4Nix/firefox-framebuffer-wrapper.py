#!/usr/bin/env python3
"""
Firefox framebuffer wrapper for Fire4Nix.
...
class CommandBatcher:
    ...
    def __init__(self, display_num=":99", logger=None):
        self.display_num = display_num
        self.proc = None
        self.batch = []
        self.last_flush_time = time.time()
        try:
            self.max_batch_age = max(0.01, float(env_value("FIRE4NIX_BATCH_MAX_AGE", "FIRE4NIX_MAX_BATCH_AGE", default="0.15")))
        except (TypeError, ValueError):
            self.max_batch_age = 0.15
        self.log_callback = logger or (lambda message: print(message, flush=True))
        self.start_process()
    ...
    def start_process(self):
        try:
            ...
            self.log_callback("xdotool is not available; command batching will remain disabled")
            self.proc = None
        except Exception as exc:
            self.log_callback(f"failed to start xdotool batching: {exc}")
            self.proc = None
...
class FirefoxFramebufferWrapper:
    def __init__(self, initial_url="https://example.com", pipe_base="fire4nix"):
        ...
        try:
            self.internal_scale = max(1, int(env_value("FIRE4NIX_INTERNAL_SCALE", "FIRE4NIX_INTERNAL_SCALE", default="1")))
        except ValueError:
        ...
        self.max_perf = env_flag("FIRE4NIX_MAX_PERF", env_flag("FIRE4NIX_MAX_PERF", False))
        self.low_quality = env_flag("FIRE4NIX_LOW_QUALITY", env_flag("FIRE4NIX_LOW_QUALITY", True))
        self.no_sleep = env_flag("FIRE4NIX_NO_SLEEP", env_flag("FIRE4NIX_NO_SLEEP", False))
        self.soc = env_value("FIRE4NIX_SOC", "FIRE4NIX_SOC", default="rk3326").lower()
    ...
    def debug(self, message):
        if env_value("FIRE4NIX_INPUT_DEBUG", "FIRE4NIX_INPUT_DEBUG"):
    ...
    def find_firefox(self):
        # Fire4Nix can run against Firefox when it exists, but ROCKNIX builds
        # may ship a compatible browser binary under a different name.
        candidates = [
            env_value("FIRE4NIX_BROWSER_BINARY", "FIRE4NIX_BROWSER_BINARY", default=None),
            "/usr/bin/firefox",
            ...
            "/usr/bin/browser.arm64",
            "/usr/local/bin/browser.arm64",
            "/usr/bin/fire4nix",
            "/usr/local/bin/fire4nix",
            "/usr/bin/fire4nix",
            "/usr/local/bin/fire4nix",
            "firefox-esr",
            "firefox",
    ...
    def detect_backends(self):
        ...
        if self.which("xdotool"):
            ...
            self.command_batcher = CommandBatcher(self.display, logger=self.log)
    ...
    def firefox_env(self):
        ...
        if self.apulse_bin:
            ...
            if lib_path:
                ...
                pass 
        env["FIRE4NIX_USER_AGENT"] = env_value(
            "FIRE4NIX_USER_AGENT",
            "FIRE4NIX_USER_AGENT",
            default="Mozilla/5.0 (X11; Linux aarch64; rv:128.0) Gecko/20100101 Firefox/128.0",
        )
        env["FIRE4NIX_USER_AGENT"] = env["FIRE4NIX_USER_AGENT"]
        env["FIRE4NIX_AUDIO_BACKEND"] = env_value("FIRE4NIX_AUDIO_BACKEND", "FIRE4NIX_AUDIO_BACKEND", default="auto")
        env["FIRE4NIX_AUDIO_BACKEND"] = env["FIRE4NIX_AUDIO_BACKEND"]
    ...
    def ensure_pulseaudio(self):
        ...
        if apulse_bin and env_value("FIRE4NIX_USE_APULSE", "FIRE4NIX_USE_APULSE", default="1") == "1":
    ...
    def start_firefox(self):
        ...
        audio_backend = env_value("FIRE4NIX_AUDIO_BACKEND", "FIRE4NIX_AUDIO_BACKEND", default="auto").strip().lower()
    ...
    "Preferences": {
      ...
      "media.cubeb.backend": { "Value": "pulse", "Status": "locked" }
        ...
        if taskset and self.is_linux:
            ...
            cpu_set = env_value("FIRE4NIX_CPUSET", "FIRE4NIX_CPUSET", default="").strip()
            if not cpu_set:
                if self.is_rk3326:
        ...
        try:
            ...
            self.firefox_log = open("/tmp/fire4nix_firefox.log", "w")
            ...
            self.log(f"Browser PID: {self.firefox_process.pid}")
            return True
        except Exception as exc:
...
if __name__ == "__main__":
    ...
    pipe_base = sys.argv[2] if len(sys.argv) > 2 else "fire4nix"
