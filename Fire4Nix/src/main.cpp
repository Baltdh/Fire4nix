...
static const char* envAny(const char* primary, const char* fallback);
...
static void initLogging() {
...
#ifdef _WIN32
    if (path.rfind("/tmp/", 0) == 0) path = std::string(".") + "/fire4nix.log";
#endif
    std::error_code ec;
    ...
    if (g_logFile) {
        const char* referenceRoot = std::getenv("FIRE4NIX_REFERENCE_ROOT");
        ...
        g_logFile << "[I] " << nowString() << " - Logger started for Fire4Nix " << FIRE4NIX_VERSION
                  << " on " << FIRE4NIX_PLATFORM
                  << " reference=" << (referenceRoot ? referenceRoot : "bundled")
                  << " summary=" << fire4nix::referencePackSummary()
                  << "\n";
        if (referenceManifest != nullptr && *referenceManifest != '\0') {
            g_logFile << "[I] " << nowString() << " - reference_manifest=" << referenceManifest << "\n";
        }
        if (referenceEnv != nullptr && *referenceEnv != '\0') {
            g_logFile << "[I] " << nowString() << " - reference_env=" << referenceEnv << "\n";
        }
        if (referenceFeatures != nullptr && *referenceFeatures != '\0') {
            g_logFile << "[I] " << nowString() << " - reference_features=" << referenceFeatures << "\n";
        }
        if (referenceStatus != nullptr && *referenceStatus != '\0') {
            g_logFile << "[I] " << nowString() << " - reference_status=" << referenceStatus << "\n";
        }
...
static constexpr const char* SHM_PATH = "/dev/shm/fire4nix_fb";
...
private:
    std::optional<std::string> findWrapperPath() const {
        if (const char* env = std::getenv("FIRE4NIX_WRAPPER"); env != nullptr && *env != '\0') {
            return std::string(env);
        }
        ...
        std::vector<std::filesystem::path> candidates = {
            ...
            "/opt/fire4nix/firefox-framebuffer-wrapper.py"
        };
    ...
    bool launchFirefox(const std::string& url) {
        ...
        if (!fbReader_.initialize("fire4nix")) {
        ...
        // This will succeed once the Python wrapper creates /dev/shm/fire4nix_fb
...
#ifdef _WIN32
        std::string cmdline = "py -3 \"" + *wrapperPath + "\" \"" + url + "\" fire4nix";
        STARTUPINFOA si = {};
        ...
        if (pid == 0) {
            ...
            execlp("python3", "python3", wrapperPath->c_str(), url.c_str(), "fire4nix", nullptr);
            
            // If Python not available, try with python
            execlp("python", "python", wrapperPath->c_str(), url.c_str(), "fire4nix", nullptr);
...
public:
    explicit App(LaunchOptions options)
        ...
        maxPerformance_ = envFlagEnabled("FIRE4NIX_MAX_PERF", false);
        forceVsync_ = envFlagEnabled("FIRE4NIX_FORCE_VSYNC", false);
        noSleep_ = envFlagEnabled("FIRE4NIX_NO_SLEEP", false);
        frameSkip_ = std::max(1, envInt("FIRE4NIX_FRAME_SKIP", 2));
        volumeStepPercent_ = std::clamp(envInt("FIRE4NIX_VOLUME_STEP", 5), 1, 20);
...
private:
    ...
    void handleControllerButton(SDL_GameControllerButton button, bool down) {
        static bool debugInput = (std::getenv("FIRE4NIX_DEBUG_INPUT") != nullptr);
        if (debugInput && down) {
            std::ostringstream ss;
    ...
    void handleJoyHat(std::uint8_t value) {
        if (value & SDL_HAT_UP) {
            if (hasActiveKeyboard()) {
                moveKeyboardSelection(0, -1);
                return;
    ...
    void handleJoyButton(std::uint8_t button, SDL_JoystickID instanceId, bool down) {
        static bool debugInput = (std::getenv("FIRE4NIX_DEBUG_INPUT") != nullptr);
        if (debugInput && down) {
            std::ostringstream ss;
        ...
        if (framesReceived_ == 0) {
            if (loadingOverlayTexture_ == nullptr || loadingOverlayWidth_ != width || loadingOverlayHeight_ != height || loadingOverlayCachedSeconds_ != loadingOverlayCurrentSeconds_) {
                ...
                if (loadingOverlayTexture_ != nullptr) {
                    ...
                    std::string msg = "FIRE4NIX";
                    std::string sub = "Starting browser " + std::to_string(loadingOverlayCurrentSeconds_) + "s";
    ...
    void adjustSystemVolume(int deltaPercent) {
        ...
        static bool debugAudio = (std::getenv("FIRE4NIX_DEBUG_AUDIO") != nullptr && std::string(std::getenv("FIRE4NIX_DEBUG_AUDIO")) == "1");
...
int main(int argc, char** argv) {
    if (argc > 1 && argv[1] != nullptr) {
        ...
        if (firstArg == "--help" || firstArg == "-h") {
            std::cout << "Fire4Nix Browser " << FIRE4NIX_VERSION << " (" << FIRE4NIX_PLATFORM << ")\n";
            std::cout << "Usage: " << (argv[0] != nullptr ? argv[0] : "fire4nix") << " [URL] [--private] [--automation] [--ignore-tls-errors]\n";
            return 0;
        }
    ...
    // Initialize logging system (can be overridden with FIRE4NIX_LOG / FIRE4NIX_LOG)
    initLogging();
    std::cout << "Fire4Nix Browser " << FIRE4NIX_VERSION << " (" << FIRE4NIX_PLATFORM << ")\n";
    logInfo(std::string("Fire4Nix Browser ") + FIRE4NIX_VERSION + " started on " + FIRE4NIX_PLATFORM);

    LaunchOptions options;
    const bool hasCommandLineUrl = argc > 1 && argv[1] != nullptr && std::strlen(argv[1]) > 0;
    if (hasCommandLineUrl) {
    ...
    if (!hasCommandLineUrl) {
        if (const char* homeUrl = std::getenv("FIRE4NIX_HOME_URL"); homeUrl != nullptr && *homeUrl != '\0') {
            options.initialUrl = homeUrl;
        }
    }
    if (options.initialUrl.empty()) {
        options.initialUrl = "about:home";
    }
