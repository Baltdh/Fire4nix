...
for pkg in wpe-webkit-2.0 wpe-platform-wayland-2.0 wpe-platform-2.0 wpe-webkit wpewebkit libwpe-1.0 wpebackend-fdo-1.0; do
  if "$PC" --exists "$pkg" >/dev/null 2>&1; then
    ...
    found=1
    case "$pkg" in wpe-webkit-2.0|wpe-webkit|wpewebkit) [ -n "$webkit_pkg" ] || webkit_pkg="$pkg" ;; esac
  else
    echo "$pkg=missing"
  fi
done
if [ -n "$webkit_pkg" ]; then
  if "$PC" --atleast-version=2.52.6 "$webkit_pkg" >/dev/null 2>&1; then echo "security_baseline=ok"; else echo "security_baseline=outdated"; fi
fi
